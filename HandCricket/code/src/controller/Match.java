...
public class Match {
	...
	public boolean isInningsCompleted() {
	...
	public void out(int idx) {
		totalWickets++;
		teamA[idx].setGotOut(true);
		idx = max(onStrike, nonStriker) + 1;
		this.onStrike = idx;
		if (max_wickets == totalWickets) {
	...
	public void initializeBatsman(int idx, int arr[]) {
		teamA[idx].setBatsmanStats(arr);
	}
	public void initializeBowler(int idx, int arr[]) {
		teamB[idx].setBowlerStats(arr);
	}
	...
	public void ball(int a) {
		this.randball = bowl();
		if(teamA[onStrike].didGetChance() == false) {
			newBatsman.setText("Batsman Stats:\nName : "+teamA[onStrike].getName()+"\nRuns : "+teamA[onStrike].getPrevRuns() +"\nBalls : "+teamA[onStrike].getPrevBalls()+"\nHighest : "+teamA[onStrike].getPrevHighest()+"\nFifties : "+teamA[onStrike].getPrevFifties()+"\nHundreds : "+teamA[onStrike].getPrevHundreds()+" ");
		}
		else {
			newBatsman.setText("Batsman Stats:");
		}
		if(teamB[bowler].gotChance==false) {
			newBowler.setText("Bowler Stats:\nName : "+teamB[bowler].getName()+"\nRuns Conceded : "+teamB[bowler].getPrevRuns()+"\nBalls Bowled : "+teamB[bowler].getPrevBalls()+"\nBest Bowling : "+teamB[bowler].getPrevBestWickets()+" / "+teamB[bowler].getPrevBestRuns()+" ");
		} else {
			newBowler.setText("Bowler Stats:");
		}
		teamA[onStrike].setBalls(teamA[onStrike].getBalls() + 1);
		teamB[bowler].setBalls(teamB[bowler].getBalls() + 1);;
		teamA[onStrike].setGotChance(true);
		teamB[bowler].setGotChance(true);
		...
		displayScore.setText(teamBat+" : "+totalScore+" / "+totalWickets+"    "+teamA[onStrike].getName()+" "+teamA[onStrike].getRuns()+" ("+teamA[onStrike].getBalls()+")*  "+teamA[nonStriker].getName()+" "+teamA[nonStriker].getRuns()+" ("+teamA[nonStriker].getBalls()+")  "+teamB[bowler].getName()+" "+teamB[bowler].getWickets()+" / "+teamB[bowler].getRuns());
		else
